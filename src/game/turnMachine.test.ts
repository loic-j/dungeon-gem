...
import { initCombat, processManaPhase, processPlayerAction, processMonsterPhase, checkCombatEnd, resetCombat } from './turnMachine'
...
describe('processManaPhase', () => {
  it('adds 1 mana to player pool', () => {
    ...
    expect(next.player.manaPool.length).toBe(Math.min(before + 1, state.player.maxMana))
  })
...
describe('processPlayerAction', () => {
  ...
  it('valid spell: mana consumed, monster HP reduced', () => {
    const state = { ...processManaPhase(initCombat()), player: { ...processManaPhase(initCombat()).player, manaPool: ['fire' as const, 'water' as const] } }
    const before = state.player.manaPool.length
...
describe('processMonsterPhase', () => {
  it('attack occurred → AP reset to 0', () => {
    ...
    const state = { ...initCombat(), monster: { ...initCombat().monster, actionPoints: 3 } }
  ...
  it('no attack → AP unchanged', () => {
    ...
    const state = { ...initCombat(), monster: { ...initCombat().monster, actionPoints: 1 } }
    const { attacked, state: next } = processMonsterPhase(state)
...
describe('checkCombatEnd', () => {
  it('GAME_OVER when player HP ≤ 0', () => {
    const state = { ...initCombat(), player: { ...initCombat().player, hp: 0 } }
    expect(checkCombatEnd(state)).toBe('GAME_OVER')
  })

  it('VICTORY when monster HP ≤ 0', () => {
    const state = { ...initCombat(), monster: { ...initCombat().monster, hp: 0 } }
...
describe('resetCombat', () => {
  it('monster returns to 10/10 HP', () => {
    const state = { ...initCombat(), monster: { ...initCombat().monster, hp: 2 } }
  ...
  it('monster AP reset to 0', () => {
    const state = { ...initCombat(), monster: { ...initCombat().monster, actionPoints: 5 } }
    const next = resetCombat(state)
    expect(next.monster.actionPoints).toBe(0)
  })

  it('mana pool reset to 1 token', () => {
    const state = { ...initCombat(), player: { ...initCombat().player, manaPool: ['fire' as const, 'water' as const, 'nature' as const] } }
    const next = resetCombat(state)
